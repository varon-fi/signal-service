...
from varon_fi.ta import atr, ema
...
class AtrBreakoutStrategy(BaseStrategy):
    ...
    def on_candle(self, candle: dict, history: pd.DataFrame) -> Optional[Signal]:
        ...
        # EMA filter
        ema_vals = ema(closes, ema_filter)

        # Bands calculation
        highest_high = pd.Series(highs).rolling(atr_length).max().values
        lowest_low = pd.Series(lows).rolling(atr_length).min().values

        upper_band = highest_high + atr_vals * atr_mult
        lower_band = lowest_low - atr_vals * atr_mult

        if pd.isna(upper_band[-1]) or pd.isna(ema_vals[-1]):
            return None
