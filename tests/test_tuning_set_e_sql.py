...
def test_tuning_set_e_sql_targets_expected_strategy_and_params():
    sql_path = Path(__file__).resolve().parents[1] / "scripts" / "tuning_set_e_rmr_xrp_5m.sql"
    ...
    # Single-variable timing control delta
    assert "'ema_filter_period', 30" in sql

    # Other entry controls pinned
